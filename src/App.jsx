...
function PriorityCard({ priority, onToggle }) {
  return (
    <div onClick={()=>onToggle(priority.id)} style={{background:'#1a1a1a',borderRadius:'16px',padding:'16px',marginBottom:'12px',cursor:'pointer',opacity:priority.done?0.5:1,transition:'opacity 0.2s'}}>
      <div style={{display:'flex',alignItems:'flex-start',gap:'12px'}}>
        <div style={{width:'22px',height:'22px',borderRadius:'50%',border:priority.done?'none':'2px solid #444',background:priority.done?'#22c55e':'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:'2px'}}>
          {priority.done && <span style={{color:'white',fontSize:'12px'}}>✓</span>}
        </div>
        <div style={{flex:1}}>
          <p style={{margin:0,fontSize:'14px',fontWeight:'600',color:priority.done?'#555':'white',textDecoration:priority.done?'line-through':'none'}}>{priority.title}</p>
          <p style={{margin:'4px 0 0',fontSize:'12px',color:'#666'}}>→ {priority.reason}</p>
        </div>
        <span style={{fontSize:'11px',padding:'3px 10px',borderRadius:'20px',background:'#2a2a2a',color:'#777',flexShrink:0}}>{priority.pillar}</span>
      </div>
    </div>
  )
}
...
function ModuleCard({ module, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{background:'#1a1a1a',borderRadius:'16px',padding:'14px 8px',display:'flex',flexDirection:'column',alignItems:'center',cursor:onClick?'pointer':'default',gap:'4px'}}>
...
export default function App() {
  ...
  const modules = [
    ...
    {id:3,name:"Finances",icon:"💰",score:financeScore},
    {id:4,name:"Orga",icon:"📋",score:65},
  ]
  const globalScore = Math.round((healthScore+sleepScore+financeScore+65)/4)
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0a0a',color:'white'}}>
      ...
      <div style={{maxWidth:'420px',margin:'0 auto',padding:'32px 20px'}}>
        ...
        {!sleepDone ? (
          <div onClick={()=>setShowCheckin(true)} style={{background:'#0f172a',border:'1px solid #1e293b',borderRadius:'16px',padding:'16px',marginBottom:'24px',cursor:'pointer',display:'flex',alignItems:'center',gap:'12px'}}>
        ...
        <div style={{marginBottom:'32px'}}>
          <p style={{margin:'0 0 12px',fontSize:'12px',fontWeight:'600',color:'#555',letterSpacing:'0.08em',textTransform:'uppercase'}}>Tes priorités du jour</p>
          {priorities.map(p=><PriorityCard key={p.id} priority={p} onToggle={togglePriority}/>)}
        </div>

        <div>
          <p style={{margin:'0 0 12px',fontSize:'12px',fontWeight:'600',color:'#555',letterSpacing:'0.08em',textTransform:'uppercase'}}>Mes modules</p>
          <div style={{display:'grid',gridTemplateColumns:'repeat(4, 1fr)',gap:'8px'}}>
            {modules.map(m=>(
              <ModuleCard key={m.id} module={m}
                onClick={
                  m.id===1?()=>setShowBloodTest(true):
                  m.id===2?()=>setShowCheckin(true):
                  m.id===3?()=>setShowFinance(true):undefined
                }
              />
            ))}
          </div>
